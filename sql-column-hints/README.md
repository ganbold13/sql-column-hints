...
Instead of counting commas across a 24-column insert:

```sql
INSERT INTO `wallet` (`id`, `user_id`, `wallet_rule_id`, ...) VALUES
('93', '1', '11', ...),
```

the editor renders (hints are virtual text — the file on disk is untouched):

```sql
('93', '1', '11', ...)
 ^id:  ^user_id: ^wallet_rule_id:
```

It also flags rows whose value count doesn't match the column list, which is the bug that inline column names are usually being counted out by hand to find.
